...
# ฟังก์ชันคำนวณพลศาสตร์ Cg (แก้ Bug ให้ L=ตั้งฉากลม, B=ขนานลม ตรงตาม UI 100%)
def calculate_dynamic_cg(H, B, L, T1, damping, exposure, V):
    n1 = 1.0 / T1 if T1 > 0 else 1.0
    z_bar = max(0.6 * H, 4.5) 
    
    if 'A' in exposure: c, ell, epsilon, alpha_bar, b_bar = 0.30, 98.0, 0.33, 0.15, 0.65
    elif 'B' in exposure: c, ell, epsilon, alpha_bar, b_bar = 0.20, 152.0, 0.25, 0.25, 0.45
    else: c, ell, epsilon, alpha_bar, b_bar = 0.15, 198.0, 0.20, 0.35, 0.30
        
    Iz_bar = c * ((10.0 / z_bar) ** (1.0 / 6.0))
    Lz_bar = ell * ((z_bar / 10.0) ** epsilon)
    
    # Q: ใช้ความกว้างรับลม (L) เข้ามาคำนวณ
    ...
    # eta_B ใช้ L (ตั้งฉากลม), eta_L ใช้ B (ขนานลม) ตามนิยาม
    eta_h = (4.6 * n1 * H) / V_bar_z if V_bar_z > 0 else 0.1
    eta_B = (4.6 * n1 * L) / V_bar_z if V_bar_z > 0 else 0.1 
    eta_L = (15.4 * n1 * B) / V_bar_z if V_bar_z > 0 else 0.1 
...
# Streamlit UI Setup
# ==========================================
st.set_page_config(page_title="Wind Load Analyzer Pro V3.1", layout="wide")
...
st.markdown('<div class="main-title">🌪️ Wind Load Analyzer Pro V3.1 (3D Vision)</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-title">ระบบวิเคราะห์แรงลมสุทธิพลศาสตร์, แผนภาพ 3D, และรายการคำนวณตาม มยผ. 1311-50</div>', unsafe_allow_html=True)
...
if "Flexible" in gust_mode:
    T1_input = st.sidebar.number_input("คาบการสั่นธรรมชาติขั้นพื้นฐาน T1 (วินาที)", value=1.20, min_value=0.1, step=0.05)
    damping_input = st.sidebar.number_input("อัตราส่วนความหน่วง (Damping Ratio)", value=0.020, min_value=0.005, max_value=0.05, step=0.005, format="%.3f")
else:
    Cg_input = 2.0
...
with st.expander("📐 ปรับแต่งความสูงแยกรายชั้น", expanded=True):
...
# ==========================================
# 3D MODEL & 2D PLOTLY FUNCTIONS
# ==========================================

def plot_3d_building(B, L, H_total, roof_type, h_roof):
    fig = go.Figure()
    
    # พิกัดกล่องอาคารหลัก
    x_pts = [0, B, B, 0, 0, B, B, 0]
    y_pts = [0, 0, L, L, 0, 0, L, L]
    z_pts = [0, 0, 0, 0, H_total, H_total, H_total, H_total]
    
    if "จั่ว" in roof_type:
        x_pts.extend([B/2, B/2])
        y_pts.extend([0, L])
        z_pts.extend([H_total + h_roof, H_total + h_roof])
        
    # วาดพื้นผิวอาคารโปร่งแสง (Convex Hull)
    fig.add_trace(go.Mesh3d(
        x=x_pts, y=y_pts, z=z_pts,
        alphahull=0, opacity=0.15, color='#3B82F6', name='พื้นที่อาคาร'
    ))
    
    # วาดโครงเส้น (Wireframe) ให้รูปทรงคมชัด
    line_style = dict(color='#1E3A8A', width=3)
    fig.add_trace(go.Scatter3d(x=[0, B, B, 0, 0], y=[0, 0, L, L, 0], z=[0, 0, 0, 0, 0], mode='lines', line=line_style, name='โครงร่าง'))
    fig.add_trace(go.Scatter3d(x=[0, B, B, 0, 0], y=[0, 0, L, L, 0], z=[H_total, H_total, H_total, H_total, H_total], mode='lines', line=line_style, showlegend=False))
    for i, j in zip([0, B, B, 0], [0, 0, L, L]):
        fig.add_trace(go.Scatter3d(x=[i, i], y=[j, j], z=[0, H_total], mode='lines', line=line_style, showlegend=False))
        
    if "จั่ว" in roof_type:
        fig.add_trace(go.Scatter3d(x=[0, B/2, B], y=[0, 0, 0], z=[H_total, H_total+h_roof, H_total], mode='lines', line=line_style, showlegend=False))
        fig.add_trace(go.Scatter3d(x=[0, B/2, B], y=[L, L, L], z=[H_total, H_total+h_roof, H_total], mode='lines', line=line_style, showlegend=False))
        fig.add_trace(go.Scatter3d(x=[B/2, B/2], y=[0, L], z=[H_total+h_roof, H_total+h_roof], mode='lines', line=line_style, showlegend=False))

    # วาดลูกศรทิศทางลม 3 มิติ (Wind Direction) วิ่งเข้าแกน X=0
    wind_x = [-max(B, L)*0.3] * 3
    wind_y = [L*0.2, L*0.5, L*0.8]
    wind_z = [H_total/2] * 3
    
    fig.add_trace(go.Cone(
        x=wind_x, y=wind_y, z=wind_z,
        u=[max(B, L)*0.3]*3, v=[0]*3, w=[0]*3,
        colorscale=[[0, '#DC2626'], [1, '#DC2626']], showscale=False,
        sizemode="absolute", sizeref=max(B, L, H_total)*0.1,
        name='ทิศทางลมปะทะ'
    ))
    
    # ใส่ข้อความกำกับ L, B และ ลม
    fig.add_trace(go.Scatter3d(
        x=[B/2, -B*0.1, -max(B,L)*0.3], 
        y=[-max(L,B)*0.1, L/2, L/2], 
        z=[0, 0, H_total*1.05],
        mode='text',
        text=[f'B = {B}m<br>(ขนานลม)', f'L = {L}m<br>(ตั้งฉากลม)', 'ลมพัดเข้าด้านนี้ 🌬️'],
        textfont=dict(color=['#2563EB', '#D97706', '#DC2626'], size=13),
        showlegend=False
    ))

    fig.update_layout(
        scene=dict(
            xaxis_title='X (แนวลมพัด)', yaxis_title='Y (แนวกว้าง)', zaxis_title='Z (ความสูง)',
            aspectmode='data', # รักษาอัตราส่วนจริงให้ไม่เบี้ยว
            camera=dict(eye=dict(x=-1.5, y=-1.5, z=1.2))
        ),
        margin=dict(l=0, r=0, b=0, t=40),
        title="<b>👁️ แผนภาพ 3 มิติ (แสดงมิติตึก L, B และทิศทางลม)</b>", height=450
    )
    return fig
...
with tab1:
    ...
    # โชว์โมเดล 3D ให้เห็นชัดเจน
    st.plotly_chart(plot_3d_building(B, L, H_total, roof_type, h_roof), use_container_width=True)
    st.markdown("---")
    
    # แผนภาพหน้าตัด 2D
    view_opt = st.radio("🔘 **เลือกกรณีโหลด (Load Case):**", ["External", "Case 1", "Case 2"], horizontal=True)
    unit_opt = st.radio("📐 **เลือกหน่วยแสดงผลบนหน้าตัด:**", ["kgf/m² (หน่วยแรงกระจาย)", "kN (แรงลัพธ์แบบจุด Point Load)"], horizontal=True)
...
with tab3:
    ...
    with c_inf2:
        st.markdown(f"**มิติตัวตึก (L × B):** {L} ม. (ตั้งฉากลม) × {B} ม. (ขนานลม)")
    ...
    if dyn_components:
        ...
        st.latex(r"Q = \sqrt{\frac{1}{1 + 0.63 \left(\frac{L+H}{L_{\bar{z}}}\right)^{0.63}}} = " + f"{dyn_components['Q']:.4f} \\quad [Background]")
    ...
    for idx, f in enumerate(floors_data):
        ...
        with st.expander(f"🔹 ชั้น {f['floor']} | พิกัดความสูง z = {f['z_bot']:.2f} ถึง {f['z_top']:.2f} ม. (โหมด: {view_opt})"):
            st.markdown(f"• **พื้นที่รับลมทางดิ่ง ($A = L \\times h$):** `{f['area_front']:.2f}` $m^2$")
