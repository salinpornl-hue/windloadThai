...
def get_Ce_details(z, exposure):
    ...
    if 'A' in exposure:
        ...
        formula_str = f"max(min((z/10)^0.20, 1.5), 0.9)"
    elif 'B' in exposure:
        alpha = 0.28
        min_val, max_val = 0.7, 1.2
        formula_str = f"max(min((z/10)^0.28, 1.2), 0.7)"
    else:
        alpha = 0.40
        min_val, max_val = 0.5, 1.0
        formula_str = f"max(min((z/10)^0.40, 1.0), 0.5)"
...
if st.button("🚀 ประมวลผลและสร้างเล่มรายการคำนวณอย่างละเอียด", use_container_width=True):
    ...
    for i in range(num_stories):
        ...
        z_mid = (z_bottom + z_top) / 2.0 # คำนวณหา Ce ที่กึ่งกลางชั้นเพื่อตัวแทนแรงดันลมของชั้นนั้น
    ...
    # คำนวณแรงลมด้านอื่น ๆ (ด้านตามลมและภายในใช้ qh ที่ความสูงยอดอาคารตามมาตรฐาน)
    ...
    # วาดเส้นแบ่งชั้นและมิติตามความจริง
    for f in floors_data:
        ...
        # วาดลูกศรแรงลม Windward เข้าแต่ละชั้น (อิงตามจุดกึ่งกลางชั้น)
        arrow_scale = 1 + (f['p_windward'] / 50.0) # ปรับขนาดลูกศรตามแรงดันจริงเพื่อความสวยงาม
    ...
    # วาดแรงลม Leeward (ด้านตามลม - คงที่ตลอดความสูงอาคารอิงตามค่าที่ยอด)
    ...
    # ปรับแต่ง Layout ของกราฟให้แสดงสเกลและมิติครบถ้วน
    fig.update_layout(
    ...
    * สัมประสิทธิ์แรงดันภายนอกด้านรับลม ($C_p \\text{{ Windward}}$) = `{Cp_w:.2f}`
    ...
    # ส่วนที่ 4: การคำนวณแยกชั้นสำหรับ Windward
    st.markdown("### **[ส่วนที่ 4]: การคำนวณหน่วยแรงลมภายนอกด้านรับลมแยกรายชั้น (Windward Pressure Profile)**")
    st.markdown("สมการคำนวณแรงลมภายนอก: $$p_{ext} = I_w \\cdot q \\cdot C_e \\cdot C_g \\cdot C_p$$")
    ...
    # วนลูปแจกแจงสูตรคณิตศาสตร์แบบละเอียดยิบรายชั้น
    for f in floors_data:
    ...
    # ส่วนที่ 5: คำนวณ Leeward & Internal
    st.markdown(f"""
    ---
    ...
    * **3. คำนวณหน่วยแรงลมภายในอาคาร (Internal Pressure: $p_{{int}}$)**
    ...
    # ส่วนที่ 6: ตารางสรุปโหลดสุทธิเพื่อนำไปป้อนซอฟต์แวร์วิเคราะห์โครงสร้าง
    st.markdown("---")
    st.markdown("### **[ส่วนที่ 6]: สรุปผลหน่วยแรงลมสุทธิแยกตาม Load Cases สำหรับนำไปคำนวณออกแบบ**")
    st.markdown("ในการออกแบบ วิศวกรต้องนำกรณีแรงลมภายนอกมาหักล้าง/รวมกับแรงลมภายในอาคาร ($p_{{net}} = p_{{ext}} - p_{{int}}$) ซึ่งจะแบ่งออกเป็น 2 กรณีหลักที่วิกฤตที่สุดดังนี้:")
    
    # สร้างข้อมูลสรุปส่งออกเป็นตาราง
    summary_rows = []
    for f in floors_data:
        p_w = f['p_windward']
        # กรณีลมอัดเข้า + ลมภายในดูดช่วยกระทำทิศเดียวกัน
        net_w_case1 = p_w - p_internal_neg 
        # กรณีลมดันเข้า + ลมภายในต้านดันสวนทาง
